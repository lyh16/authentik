...
func TestForwardHandleTraefik_Single_Blank(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "/goauthentik.io/auth/traefik", nil)
...
func TestForwardHandleTraefik_Single_Headers(t *testing.T) {
	...
	assert.Equal(t, loc.String(), "http://test.goauthentik.io/goauthentik.io/start")
...
func TestForwardHandleTraefik_Domain_Header(t *testing.T) {
	...
	loc, _ := rr.Result().Location()
	assert.Equal(t, "http://auth.test.goauthentik.io/goauthentik.io/start", loc.String())
