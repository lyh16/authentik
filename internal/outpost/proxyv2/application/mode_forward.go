...
func (a *Application) configureForward() error {
	a.mux.HandleFunc("/goauthentik.io/auth", func(rw http.ResponseWriter, r *http.Request) {
	...
	a.mux.HandleFunc("/goauthentik.io/auth/traefik", a.forwardHandleTraefik)
	a.mux.HandleFunc("/goauthentik.io/auth/nginx", a.forwardHandleNginx)
	return nil
}
...
func (a *Application) forwardHandleTraefik(rw http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(r.Header.Get("X-Forwarded-Uri"), "/goauthentik.io") {
		a.log.WithField("url", r.URL.String()).Trace("path begins with /goauthentik.io, allowing access")
		return
	}
	host := ""
	...
	rdFinal := fmt.Sprintf("%s//%s%s", proto, host, "/goauthentik.io/start")
...
func (a *Application) forwardHandleNginx(rw http.ResponseWriter, r *http.Request) {
	...
	if fwd.String() != r.URL.String() {
		if strings.HasPrefix(fwd.Path, "/goauthentik.io") {
			a.log.WithField("url", r.URL.String()).Trace("path begins with /goauthentik.io, allowing access")
			return
		}
	}
