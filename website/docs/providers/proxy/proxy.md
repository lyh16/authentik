...
When using single-application mode, navigate to `app.domain.tld/goauthentik.io/sign_out`.

When using domain-level mode, navigate to `auth.domain.tld/goauthentik.io/sign_out`, where auth.domain.tld is the external host configured for the provider.

To log out, navigate to `/goauthentik.io/sign_out`.
